...
# SNP distributions on chromosomes
...
ggplot (as) + geom_bar(mapping = aes(x = Chromosome, fill = Chromosome)) +xlab(label = "Chromosome") + ylab(label = "SNPs") + ggtitle("SNP Distribution on Chromosomes - Maize") + scale_x_discrete(limit=c(1:10))

as <- teosinte_merged
as <- as[as$Chromosome %in% c(1:10),]
as <- as[as$Position != "multiple",]
as <- as[!is.na(as$Position),]
ggplot (data = teosinte_merged) + geom_bar(mapping = aes(x = Chromosome, fill = Chromosome)) +xlab(label = "Chromosome") + ylab(label = "SNPs") + ggtitle("SNP Distribution on Chromomosomes - Teosinte") + scale_x_discrete(limit=c(1:10))
```

# SNP distributions across chromosomes
```{r}
library(ggplot2)
as <- maize_merged
as <- as[as$Chromosome %in% c(1:10),]
...
ggplot(heterozygosity_maize) +
  geom_bar(aes(x=Group, fill = abph1.20), position = "fill")
